...
SELECT
		REGION,COUNT(PRODUCT) AS NO_OF_PRODUCTS
FROM SALES_DATA
GROUP BY REGION
--6. Display Sales Amount by Year and Product
SELECT 
	PRODUCT,YEAR,SUM(SALES_AMOUNT) AS TOTAL_SALES
FROM SALES_DATA
...
SELECT 
	REGION,AVG(SALES_AMOUNT) AS AVERAGE_SALARY
FROM SALES_DATA
GROUP BY REGION
HAVING AVG(SALES_AMOUNT)<10000
--9. Display Years with Maximum Sales Exceeding 500
SELECT 
	YEAR,MAX(SALES_AMOUNT) AS MAXIMUM_SALES
FROM SALES_DATA
GROUP BY YEAR
HAVING MAX(SALES_AMOUNT)>500
--10. Display Regions with at Least 3 Distinct Products Sold.
SELECT 
	REGION,COUNT(DISTINCT PRODUCT) AS NO_OF_PRODUCT
FROM SALES_DATA
GROUP BY REGION
HAVING COUNT(DISTINCT PRODUCT)>=3
--11. Display Years with Minimum Sales Less Than 1000
SELECT
	YEAR,MIN(SALES_AMOUNT) AS MINIMUM_SALES
FROM SALES_DATA
GROUP BY YEAR
...
SELECT 
	REGION,YEAR,COUNT(PRODUCT)AS NO_OF_PRODUCT
FROM SALES_DATA
...
SELECT 
	REGION,YEAR,MAX(SALES_AMOUNT) AS MAXIMUM_SALES
FROM SALES_DATA
GROUP BY REGION,YEAR
...
SELECT 
	YEAR,SUM(SALES_AMOUNT) AS TOTAL_SALES
FROM SALES_DATA
GROUP BY YEAR
...
SELECT
	 REGION,PRODUCT,COUNT(PRODUCT) AS NO_OF_PRODUCT
FROM SALES_DATA
GROUP BY REGION,PRODUCT
HAVING COUNT( PRODUCT)>1
--5. Find out region wise highest sales amount.
SELECT 
	REGION,MAX(SALES_AMOUNT) AS MAXIMUM_SALES
FROM SALES_DATA
GROUP BY REGION
